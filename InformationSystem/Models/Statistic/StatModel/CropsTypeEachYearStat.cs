...
namespace InformationSystem.Models.Statistic.StatModel
{
    ...
    /// </summary>
    public class CropsTypeEachYearStat : IAreaStat<List<CropsTypeEachYearStatResult>>
    {
        /// <inheritdoc />
        ...
        public CropsTypeEachYearStat(int year, string cropsType = null)
        {
            int startYear;
            this._startYear = year - STAT_RANGE_YEAR + 1;
            this._endYear = year;
            this._cropsType = cropsType;
            ...
            //指定作物
            if (!string.IsNullOrEmpty(cropsType))
            {
                filter = filter.And(x => x.CropType == cropsType);
            }
        ...
        /// 指定作物
        /// </summary>
        private readonly string _cropsType;
        ...
        /// <inheritdoc />
        public string ExportFileName() => $"{this._startYear-1911}年度{this._cropsType}輔導人次、輔導面積統計結果.xlsx";
    }
}