...
namespace InformationSystem.Helper
{
    public class SmsReadFile
    {
        ...
        public string setReplacedSmsDataForMemberInitPassword(string sTxtName, string language, MemberInitPasswordReplaceModel smsReplaceData)
        {
            //輸出
            string returnData = "";
            ...
            //取代文字
            if (smsReplaceData != null)
            {
                if (!string.IsNullOrEmpty(smsReplaceData.Account))
                {
                    returnData = Regex.Replace(returnData, "#Account#", smsReplaceData.Account);
                }
                if (!string.IsNullOrEmpty(smsReplaceData.LoginPass))
                {
                    returnData = Regex.Replace(returnData, "#Password#", smsReplaceData.LoginPass);
                }
                if (!string.IsNullOrEmpty(smsReplaceData.WebUrl))
                {
                    returnData = Regex.Replace(returnData, "#WebUrl#", smsReplaceData.WebUrl);
                }
                //...無限向下增加
            }
            return returnData;
